...
const login = () => {
  ...
  const Login = async (e) => {
    ...
    try{
      ...
      await axios.post(BASE_URL + "/api/timer/login", form)
      .then(res => {
        if (res.data.logged) {
          let data = res.data.data
          let token = res.data.token
          localStorage.setItem("timerSeni", JSON.stringify(data))
          localStorage.setItem("token", token)

          router.push('./')//init router
        } else {
  ...
  return (
    ...
    <div className="w-screen h-screen flex flex-col justify-center items-center">
      <form className="flex flex-col justify-center items-center z-50" onSubmit={(e) => Login(e)}>
        ...
        <h2 className="my-8 font-display font-bold text-3xl text-gray-700 text-center">Login Timer</h2>
