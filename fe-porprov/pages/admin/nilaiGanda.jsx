...
const nilaiGanda = () => {
  ...
  const toNilaiGanda = (item) => {
    localStorage.setItem ('jadwalGandaBiru', (item.biru.id))
    localStorage.setItem ('jadwalGandaMerah', (item.merah.id))

    router.push ('./cetakGanda/' + item.id)
  }
          ...
          <div className="p-7 space-y-5">
            ...
            <div className="bg-[#2C2F48] rounded-lg flex justify-between p-3">
              <div className="flex items-center px-2">
                <span className='text-lg uppercase font-semibold'>Rekap Nilai Ganda</span>
              </div>
            </div>
            ...
            <div className="bg-[#2C2F48] min-h-full rounded-lg">
              ...
              <table className='w-full table-fixed'>
                ...
                <tbody className='text-center'>
                  {dataJadwalGanda.filter (a => a.selesai == true).map ((item, index) => (
                    <tr key={index + 1} className='even:bg-[#4C4F6D] odd:bg-[#2c2f48]'>
                      <td className='py-5'>{index + 1}</td>
                      <td>{item.partai}</td>
                      <td>{item.kelas}</td>
                      <td>{item.pemenang.nama1} - {item.pemenang.nama2} ({item.pemenang.kontingen})</td>
                      {(() => {
                      ...
                      <td>{item.babak}</td>
                      <td>
                      <button onClick={() => toNilaiGanda (item)} className="flex m-auto">
